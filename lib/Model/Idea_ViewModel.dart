...
class Idea_ViewModel extends StateNotifier<Idea> {
  ...
  List<Color> generateColorPalette(String mood) {
    ...
    switch (mood) {
      case "uplifting":
        return warmSoft[_random.nextInt(warmSoft.length)];
      case "dramatic":
        return (warmDramatic + coldDramatic)[_random.nextInt((warmDramatic + coldDramatic).length)];
      case "warm":
        return (warmDramatic + warmSoft)[_random.nextInt((warmDramatic + warmSoft).length)];
      case "sad":
        return coldSoft[_random.nextInt(coldSoft.length)];
      case "angry":
        return warmDramatic[_random.nextInt(warmDramatic.length)];
      case "calm":
        return (coldSoft + warmSoft)[_random.nextInt((coldSoft + warmSoft).length)];
      case "cold":
        return (coldSoft + coldDramatic)[_random.nextInt((coldSoft + coldDramatic).length)];
      case "scary":
        return coldDramatic[_random.nextInt(coldDramatic.length)];
      default:
        return [];
    }
  }
  ...
  String generateThemeExample() {
    ...
    switch (state.ideaTheme) {
      case "science fiction":
        return scienceFictionTypes[_random.nextInt(scienceFictionTypes.length)];
      case "fantasy":
        return fantasyTypes[_random.nextInt(fantasyTypes.length)];
      case "mitology":
        return mitologyTypes[_random.nextInt(mitologyTypes.length)];
      case "real life":
        return realLifeTypes[_random.nextInt(realLifeTypes.length)];
      default:
        return "";
    }
  }
